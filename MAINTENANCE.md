...
- `assets/link-*.svg`：三个可独立点击的按钮图片。
- `source/content.json`：软件名称、真实入口地址、动画速度配置。
- `source/figma-light.svg`：原画板导出，保留为视觉基线。
- `source/text-outlines.json`：软件词条和按钮的字体轮廓，不包含字体文件。
- `build.py`：从源文件生成 README、动效图、按钮和本地预览。
...
整张主图只链接个人网站。下面的个人网站、小红书、公众号按钮才是三个不同链接。公众号使用用户提供的公开文章，通过文章作者进入公众号；不得使用带登录 token 的后台地址。
...
发布前查看生成文件差异；发布后检查 GitHub 主页的实际图片、动画、独立链接，以及手机下的按钮换行。
